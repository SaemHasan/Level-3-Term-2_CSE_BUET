...
namespace ns3
{
  ...
  TcpSocketBase::EnterRecovery(uint32_t currentDelivered)
  {
    ...
    // std::cout << "thresh upper : " << m_tcb->m_ssThresh << std::endl;
    m_tcb->m_ssThresh = m_congestionControl->GetSsThresh(m_tcb, bytesInFlight);
    // std::cout << "thresh lower: " << m_tcb->m_ssThresh << std::endl;
  ...
  TcpSocketBase::DupAck(uint32_t currentDelivered)
  {
    ...
    // new added
    m_tcb->m_inDUPACK = true;
    m_tcb->m_DUPACKCOUNT = m_dupAckCount;
    if (m_congestionControl->GetName() == "TcpWestwoodBR")
    {
      // std::cout << "dup ack count : " << m_dupAckCount << std::endl;
      // std::cout << "cwnd : " << m_tcb->m_cWnd << std::endl;
      // m_tcb->m_cWnd = m_tcb->m_cWnd *
      // std::cout << "algo : " << m_congestionControl->GetName() << "\t L: " << m_tcb->m_inDUPACK << std::endl;
      m_congestionControl->UpdateWindow(m_tcb);
    }
    m_tcb->m_inDUPACK = false;
  ...
  TcpSocketBase::EstimateRtt(const TcpHeader &tcpHeader)
  {
    ...
    if (!m.IsZero())
    {
      ...
      m_tcb->m_maxRtt = std::max(m_tcb->m_lastRtt.Get(), m_tcb->m_maxRtt); // new added
