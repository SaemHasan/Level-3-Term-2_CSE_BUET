...
// 2. Create variables
        ...
        /* the nine blocks or boxes *//* each box is an array of 9 integer variables taking their value in [1, 9] */
...
// 3. Post constraints
        ...
        for ( j = 0; j < 9; j++)
            model.arithm (bd[8][j], "=", r8[j]).post();/* for the nine column variables */
        ...
        for ( i = 0; i < 9; i++)
            model.arithm (bd[i][5], "=", c5[i]).post();for ( i = 0; i < 9; i++)
        ...
        model.allDifferent(b8).post();// 4. Solve the problem
